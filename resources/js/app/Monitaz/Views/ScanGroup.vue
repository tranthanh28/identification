  ...
  <div class="content-wrapper">
    <div class="row">
      <div class="col-sm-12 col-md-6">
        <app-breadcrumb page-title="Scan Group" :directory="$t('datatables')" :icon="'grid'"/>
      </div>
...
export default {
  ...
  methods: {
    ...
    handleAdd() {
      axios.post('/api/scan-group', this.form).then((response) => {
        this.stopLoading()
        this.getList()
    ...
    handleUpdate() {
      axios.put(`/api/scan-group/${this.form.id}`, this.form).then((response) => {
        this.stopLoading()
        this.getList()
        this.dialogFormVisible = false
        this.form = {
    ...
    handleDowload(row) {
      ...
      axios.post('/api/scan-group/export-excel', dataDowload, {
        responseType: 'blob'
      }).then((response) => {
